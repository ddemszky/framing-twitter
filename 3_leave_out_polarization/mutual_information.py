...
def mutual_information(dem_counts, rep_counts):
    ...
    dem_t = np.zeros(no_tokens)
    rep_t = np.zeros(no_tokens)
    for k, v in dem_t_counts.items():
        dem_t[k] = v
    for k, v in rep_t_counts.items():
        rep_t[k] = v
    dem_not_t = dem_no - dem_t
    rep_not_t = rep_no - rep_t
    all_t = dem_t + rep_t
    all_not_t = no_users - all_t
...
def get_polarization(event, method = "nofilter", cluster_method = None):
    ...
    return get_values(event, data, method=mutual_information)
