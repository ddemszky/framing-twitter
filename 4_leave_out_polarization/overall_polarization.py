...
def get_polarization(event, method = "nofilter", cluster_method = None):
    ...
    data = data[(data['timestamp'].astype(int) > event_times[event]) & ~data['user_id'].isnull()]
    if method == "noRT":
        data = data[~data['isRT']]
    elif method == "cleaned":
...
if __name__ == "__main__":
    ...
    with open(DATA_DIR + 'polarization_' + method + '.json', 'w') as f:
